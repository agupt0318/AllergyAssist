...
class ViewController: UIViewController {
    ...
    private func start(){
        log_in.frame = CGRect(x : left_margin*10, y: view.frame.height * 0.55, width: view.frame.width - 20 * left_margin, height: container_height*0.25) //x : 125, y: 475, width: 150, height: 48
        view.addSubview(log_in)
        scan_barcode.frame = CGRect(x : log_in.center.x - log_in.frame.width*0.5, y: log_in.center.y + log_in.frame.height*1.3, width: view.frame.width - 20 * left_margin, height: container_height*0.25)
        view.addSubview(scan_barcode)
        scan_label.frame = CGRect(x : scan_barcode.center.x - scan_barcode.frame.width*0.5, y: scan_barcode.center.y + log_in.frame.height*1.3, width: view.frame.width - 20 * left_margin, height: container_height*0.25)
        view.addSubview(scan_label)
        appName.frame = CGRect(x : log_in.center.x - log_in.frame.width*0.5, y: log_in.center.y - log_in.frame.height*7, width: view.frame.width - 20 * left_margin, height: container_height*0.25)
        view.addSubview(appName)
        //x imageView.frame = CGRect(x : left_margin, y: 100, width: view.frame.width - 2 * left_margin, height: container_height)
        //view.addSubview(imageView)
        imageView.frame = CGRect(x: log_in.center.x - log_in.frame.width*0.92, y: view.safeAreaInsets.top + scan_label.frame.height*2.5, width: view.frame.size.width-40, height: view.frame.size.width-40)
    }
...
class ScanLabel : UIViewController, UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    ...
    @objc func pick_image(sender : UIButton){
        ...
        picker.sourceType = .camera //savedPhotosAlbum
...
class RegistrationVC : UIViewController {
    ...
    var user = User(email: "sample@gmail.com", name: "")
    ...
    let EditButton : UIButton = {
        let EB = UIButton()
        EB.setTitle("Select Allergies", for: .normal)
    ...
    private func setup(){
        EditButton.frame = CGRect(x: UserAccount.center.x - UserAccount.frame.width * 0.55, y: UserAccount
            .center.y + view.frame.width*0.75, width: UserAccount.frame.width*1.1, height: UserAccount.frame.height*0.75)
        view.addSubview(EditButton)
        //SaveButton.frame = CGRect(x : 225, y: 400, width: 50, height: 36 )
        //view.addSubview(SaveButton)
        SetUpMyAllergenProfile.frame = CGRect(x: UserAccount.center.x - UserAccount.frame.width * 0.58, y: UserAccount
            .center.y + view.frame.width*1.05, width: UserAccount.frame.width*1.2, height: UserAccount.frame.height*0.75)
        view.addSubview(SetUpMyAllergenProfile)
        UserAccount.frame = CGRect(x : left_margin*10, y: view.frame.height * 0.55, width: view.frame.width - 20 * left_margin, height: container_height*10)
    ...
    class Add_User : UIViewController {
        ...
        var user_registration = ["Email" : "sample", "Name" : "N/A", "Age" : "0"]
        //let races = ["Asian", "Black", "Latino", "White", "Other"]
        /*lazy var res = SelectRace(arr: races,
                                  n: races.count,
                                  c: UIColor.systemBlue,
    ...
    class myAllergens: UIViewController {
        ...
        @objc func h1(sender : UIButton){
            ...
            if var dt = UserDefaults.standard.dictionary(forKey: "UserDB"){
                ...
                let dt = [user.name:[[user.email],myAvoid, myLimit]]
                UserDefaults.standard.setValue(dt, forKey: "UserDB")
            }
            print(user_selection)
